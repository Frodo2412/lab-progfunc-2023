...
tests: FORCE
	...
	for i in `seq 1 $(AMOUNT_OF_ERROR_TESTS)`; \
		 do runhaskell Compiler.hs localtests/ejemplo$${i}err > \
			localtests/ejemplo$${i}err.err ; done
	for i in `seq 1 $(AMOUNT_OF_NORMAL_TESTS)`; \
		 do echo Normal test $${i}:; \
		 	diff localtests/ejemplo$$i.c tests/ejemplo$$i.c; done
	for i in `seq 1 $(AMOUNT_OF_NORMAL_TESTS)`; \
		do  echo Opt test $${i}:; \
			diff localtests/ejemplo$${i}_opt.c tests/ejemplo$${i}_opt.c; done
	for i in `seq 1 $(AMOUNT_OF_ERROR_TESTS)`; \
		do  echo Error test $${i}:; \
			diff localtests/ejemplo$${i}err.err tests/ejemplo$${i}err.err; done
